...
def count_words(subreddit, word_list, hot=[], after=None):
    """
    recursive function that queries the Reddit API, https://www.reddit.com/dev/api
    """
    ...
    response = requests.get('http://reddit.com/r/{}/hot.json'.format(subreddit), headers=meta, params=query)
...
def show_results(word_list, hot):
    ...
    items = list(i.keys())
    for item in sorted(items, reverse=True, key=lambda k: i[k]):
